...
namespace LMirman.VespaIO
{
	...
	public class Command : ICommandProperties
	{
		...
		/// </summary>
		private static readonly StringBuilder GuideBuilder = new();
        ...
        /// <inheritdoc cref="ICommandProperties.Server"/>
		...
		/// </summary>
		/// <param name="properties">The properties that define this command such as title, description properties.</param>
		...
		/// <remarks>This will <b>not</b> unset any modifications to command properties this method applied such as Name, Description, or Hidden properties.</remarks>
		...
		public bool TryGetMethod(Word[] arguments, out MethodInfo methodInfo, out object[] methodParameters)
		{
			...
			foreach (MethodInfo method in validMethods)
			{
				...
				for (int i = 0; i < parameters.Length && i < arguments.Length; i++)
				{
                    ...
                    string argumentText = arguments[i].text;
                    if (parameterType == typeof(bool) && (argumentText == "0" || argumentText == "1"))
                        argumentText = argumentText == "0" ? "false" : "true";

					if (parameterType != typeof(string))
						value++;

					TypeConverter typeConverter = TypeDescriptor.GetConverter(parameterType);
					if (!typeConverter.IsValid(argumentText) || (arguments[i].context.HasFlag(Word.Context.IsLiteral) && parameterType != typeof(string)))
					{
						canCastAll = false;
						break;
					}
			...
			else if (bestMethod != null)
			{
                ...
                {
                    string argumentText = arguments[i].text;
                    if (parameters[i].ParameterType == typeof(bool) && (argumentText == "0" || argumentText == "1"))
                        argumentText = argumentText == "0" ? "false" : "true";

                    TypeConverter typeConverter = TypeDescriptor.GetConverter(parameters[i].ParameterType);
                    try
                    {
                        methodParameters[i] = i < arguments.Length ? typeConverter.ConvertFrom(argumentText) : parameters[i].DefaultValue;
                    }
                    catch (Exception e)
                    {
                        Debug.LogError($"Error converting argument '{argumentText}' to type '{parameters[i].ParameterType}' for command '{Key}': {e.Message}\n{e.StackTrace}");
                        methodInfo = null;
                        methodParameters = null;
                        return false;
                    }
