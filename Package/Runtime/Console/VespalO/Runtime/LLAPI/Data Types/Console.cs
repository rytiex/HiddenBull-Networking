...
namespace LMirman.VespaIO
{
    ...
    {
        // Input history
        public readonly List<string> recentInputs = new(32);
        protected int inputHistoryCapacity = 16;

        // Output
        protected int outputCapacity = 8192;
        protected readonly StringBuilder output = new(16384);
        ...
        /// <summary>
        /// Optional external router for server-authoritative commands (<see cref="ICommandProperties.Server"/>).
        /// Invoked with the raw command text + resolved command just before local execution.
        /// Return true if handled externally (e.g. forwarded to the server) so it must NOT run
        /// locally; false/null = run locally. Kept engine-agnostic: the networking/console bridge
        /// assigns this; Console itself knows nothing about Mirror or game concepts.
        /// </summary>
        public Func<string, Command, bool> ServerCommandRouter { get; set; }
        ...
        {
            Invocation invocation = new(commandText, CommandSet);
            switch (invocation.validState)
            {
                case Invocation.ValidState.Valid:
                    if (invocation.command.Server && ServerCommandRouter != null && ServerCommandRouter(commandText, invocation.command))
                        break; // Handled externally (forwarded to server); do not execute locally.
        ...
        protected static string ApplyDefaultStyling(string text, LogStyling logStyling) => logStyling switch
        {
            LogStyling.Plain => text,
            LogStyling.Debug => $"<color=#6AC>[Debug]</color> {text}",
            LogStyling.Info => $"<color=#76A>[Info]</color> {text}",
            LogStyling.Notice => $"<color=#FC5>[Notice]</color> {text}",
            LogStyling.Warning => $"<color=#F94>[Warning]</color> {text}",
            LogStyling.Error => $"<color=#D55>[Error]</color> {text}",
            LogStyling.Critical => $"<color=#C22>[Critical]</color> {text}",
            LogStyling.Exception => $"<color=#D55>[Exception]</color> {text}",
            LogStyling.Assert => $"<color=#FC5>[Assert]</color> {text}",
            _ => throw new ArgumentOutOfRangeException(nameof(logStyling), logStyling, null),
        };

        private static readonly AutofillValue DefaultAutofill = new("help", 0);
        private readonly AutofillBuilder autofillBuilder = new();
        ...
        public AutofillValue GetAutofillValue(string input, HashSet<string> autofillExclusions)
        {
            ...
            string lastStatement = statements[^1];
            ...
            else if (sanitizedWords.Count == 1 && !lastStatement.EndsWith(" "))
            {
                ...
                foreach (Command command in CommandSet.GetPublicCommands())
                {
            ...
            {
                Word lastWord = sanitizedWords[^1];
                Word pureLastWord = pureWords[^1];
