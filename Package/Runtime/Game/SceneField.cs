...
#pragma warning restore 414
        ...
        public int BuildIndex
        {
            get
            {
#if UNITY_EDITOR
                {
                    ...
                    {
                        if (sceneAsset != null)
                            Debug.LogError($"SceneField: The following scene is assigned to a scene field as required, but isn't added to builds: {AssetDatabase.GetAssetPath(sceneAsset)}");
                        else
                            Debug.LogError($"SceneField: A scene field is marked as required, but no scene is assigned");
                    }
                }
#endif
...
#if UNITY_EDITOR
                {
                    sceneName = GetSceneBuildName(sceneAsset);
                    if (required && sceneName == string.Empty)
                    {
                        if (sceneAsset != null)
                            Debug.LogError($"SceneField: The following scene is assigned to a scene field as required, but isn't added to builds: {AssetDatabase.GetAssetPath(sceneAsset)}");
                        else
        ...
        void ISerializationCallbackReceiver.OnBeforeSerialize()
        {
            ...
            if (required && (buildIndex < 0 || string.IsNullOrEmpty(sceneName)))
        ...
        static Dictionary<SceneAsset, string> cachedBuildNames = new Dictionary<SceneAsset, string>();

        /// <summary>
        /// Updates the cached build indexes.
        /// </summary>
        static void UpdateCachedBuilds()
        {
            cachedBuildIndexes.Clear();
            cachedBuildNames.Clear();
            ...
            foreach (var scene in EditorBuildSettings.scenes)
            {
                if (scene.enabled)
                {
                    ...
                    if (sceneAsset != null)
                    {
                        ...
                        cachedBuildNames.Add(sceneAsset, sceneAsset.name);
                    }
                }
            }
        }
        ...
        static void OnEditorInitializeOnLoad()
        {
            ...
            EditorBuildSettings.sceneListChanged -= UpdateCachedBuilds;
            EditorBuildSettings.sceneListChanged += UpdateCachedBuilds;
        }
        ...
        /// <param name="sceneAsset">The scene asset.</param>
        /// <returns>The build index, -1 if not found.</returns>
        static string GetSceneBuildName(SceneAsset sceneAsset)
        {
            if (sceneAsset == null || !cachedBuildNames.TryGetValue(sceneAsset, out string buildName))
                return string.Empty;
            else
